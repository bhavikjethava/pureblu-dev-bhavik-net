...
const ManageCalendar = ({ open, onClose }: any) => {
  ...
  const [dropdownTechnicianList, setdropdownTechnicianList] = useState<
    Array<object>
  >([]);
  const [filteredTechnicianList, setfilteredTechnicianList] = useState<
    Array<object>
  >([]);
  ...
  // Ensure that fetchTechnicianByPartner is called when the component mounts to fetch the initial list of technicians
  useEffect(() => {
    if (selectedPartner) {
  ...
  useEffect(() => {
    const filteredList = filterTechnicians();

    setfilteredTechnicianList(filteredList);
    // Use filteredList as needed, e.g., set it to a state variable or perform other actions
    // Example: setFilteredTechnicians(filteredList);
  }, [selectedTechnicians]);
  ...
  const fetchTechnicianByPartner = async (partnerId: string) => {
    try {
      ...
      if (technician) {
        ...
        setdropdownTechnicianList(dropdownOptions);
        setresourcesTechnicianList(listOptions);
        setSelectedTechnicians(['all']); // Clear selectedTechnicians
      }
    } catch (error) {
  ...
  const filterTechnicians = () => {
    if (selectedTechnicians) {
      const selectedTech = resourcesTechnicianList.find(
        (tech: any) => tech.id === selectedTechnicians
      );
      return selectedTech ? [selectedTech] : resourcesTechnicianList;
    } else {
      return resourcesTechnicianList;
    }
  };
  ...
  return (
    <MyDialog
      ...
      ClassName='sm:max-w-[98%] h-full grow max-h-[90%]'
    >
      {loading ? (
        <Loader />
      ) : (
        <ScrollArea className='grow p-4'>
          <div className='flex h-full flex-col gap-5 bg-white p-5'>
            <div className='flex w-full flex-col items-center justify-between gap-5 lg:flex-row'>
              <div className='grid w-full grid-cols-2 gap-6 relative'>
                <div className='relative z-10'>
                  <MultiSelectDropdown
                    ...
                    onChange={(partnerId: any) =>
                      handlePartnerChange(partnerId.id)
                    }
                  />
                  {/* <SelectBox
                    className='w-full'
                    ...
                    optionValue='name' // Set the value for options
                ...
                <div className='relative '>
                  <SelectBox
                    className='w-full'
                    label=''
                    value={selectedTechnicians} // Set the value of the selected partner
