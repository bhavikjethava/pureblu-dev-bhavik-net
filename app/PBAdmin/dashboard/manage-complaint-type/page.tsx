...
import AddComplainType from './AddComplainType';
import ConfirmationDialog from '@/components/ConfirmationDialog';

interface ManageComplainTypeColumneProps {
  accessorKey: string;
  header: string;
  className?: string;
...
interface ManageComplainTypeProps {
  id?: string;
  title?: string;
  [key: string]: any;
}
...
function ManageComplainType() {
  const [showModal, setShowModal] = useState(false);
  const [selectedItem, setSelectedItem] =
    useState<ManageComplainTypeProps | null>(null);
  const [list, setList] = useState<Array<ManageComplainTypeProps>>();
  const [errors, setErrors] = useState<ManageComplainTypeProps>({});
  ...
  const handleSave = async (formData: FormData) => {
    ...
    if (isError) {
      ...
      if (selectedItem?.id) {
        ...
        const request = {
          endpoint: API_ENDPOINTS.REQUESTTYPE,
          method: 'POST',
          body: formData,
        };
  ...
  const fetchList = async (resolve?: any) => {
    try {
      const fetchRequest = {
        endpoint: `${API_ENDPOINTS.REQUESTTYPE}?need_all=1`,
        method: 'GET',
      };
  ...
  const update = async (id: string | undefined, params: any) => {
    try {
      let updateRequest = {
        endpoint: `${API_ENDPOINTS.REQUESTTYPE}/${id}`,
        method: 'PATCH',
        body: params,
      };
  ...
  const handleStatuClick = async () => {
    setLoading(true);
    let item = selectedItem as ManageComplainTypeProps;
    let tempList = list || [];
    let params = {
  ...
  const handleEditClick = (item: ManageComplainTypeProps) => {
    setErrors({});
  ...
  const columns: ManageComplainTypeColumneProps[] = [
    { accessorKey: 'name', header: 'Complaint Type' },
    { accessorKey: 'description', header: 'Description' },
    {
      accessorKey: 'Action',
      header: 'Action',
      className: '',
      render: (item: ManageComplainTypeProps) => (
...
export default ManageComplainType;
